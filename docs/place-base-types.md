...
이 문서는 `pykrtour.locations`의 기준 문서다. TripMate 하위 라이브러리가 장소,
...
`pykrtour`는 모든 라이브러리가 같은 모양의 값 객체를 주고받게 하는 경계만 담당한다.
...
- provider row의 `lon`, `lng`, `longitude`, `mapx`, `x`, `lat`, `latitude`, `mapy`,
  `y` 같은 흔한 key에서 좌표 추출
...
```python
from pykrtour import AddressRegion, JibunAddress
...
```python
from pykrtour import RoadNameAddress
...
```python
from pykrtour import AddressRegion, RoadNameAddress
